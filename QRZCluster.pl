...
# QRZCluster
# 
# Copyright (C) 2011  Better Software Solutions, Inc.
# Copyright (C) 2011  Thomas M. Schaefer, NY4I.
#
...
print "Copyright 2010 Better Software Solutions, Inc.\n";
print "This program makes use of the POE framework as well as Ham::Reference::QRZ\n";
...
if (!defined $options{'h'} || 
    !defined $options{'u'} || 
    !defined $options{'p'}){
   print "Usage: QRZCluster.pl -h <servername>[:<port>] -u <QRZ Username> -p <QRZ Password> -c MyCall for cluster -s\n";
   print "   where:\n";
   ...
   print "      REMMEBER: YOU MUST BE AT LEAST AN XML SUBSCRIBER ON QRZ FOR THIS TO WORK\n\n";
   ...
   print "   If the -c option is used with a callsign, this is the call that will be used to connect to the cluster.\n";
...
print "Connecting to DX Cluster $options{s} as $mycall\n"; 
...
sub ProcessInput {
    ...
    if ($loggedIn != 1 && $buf =~ /login:/) {
       $_[HEAP]->{server}->put('NY4I-5');
       $loggedIn = 1;
       if (defined $options{'x'}){
          sleep(5);
   ...
   if ($buf =~ /DX de/){
      ...
      if (defined $listing){
         if (!USTerritory($call)  &&
         	 $call =~ /^[AaWaKkNn][a-zA-Z]?[0-9][a-zA-Z]{1,3}$/ || # US Callsign
             $call =~ /^(CY[09]|VA][1-7]|VE[1-9]|V[OY][12]|VY[012])[a-zA-Z]{1,3}$/) { #VE Callsign
      	    if (defined $options{'s'}) { 
			   ...
			   if (defined ($listing->{state})) {
      	       ...
      	       my $section = $qrz->get_arrl_section();
			   $newComment = "[$section]  $comment";
			}
		 } else { # No US/VE callsign
		   $newComment = "[".$listing->{land}."] $comment";
		 }
...
sub USTerritory() {
	my ($call) = @_;
	return (1) if ($call =~ /^KG4/); # Gitmo
	return (1) if ($call =~ /^[AKNW]H0/); # Mariana Island
...
#    ZR8,ZS8,ZT8,ZU8;
 ...
 AH0, KH0, NH0, WH0 	Mariana Island
AH1, KH1, NH1,WH1 	American Phoenix
AH2, KH2, KG6, NH2, WH2 	Guam
AH3, KH3, NH3, WH3 	Johnston Island
AH4, KH4, NH4, WH4 	Midway Island
AH5, KH5, NH5, WH5 	Palmyra Island
AH5K, KH5K, NH5K, WH5K 	Kingman-Reef
AH6,7, KH6,7, NH6,7, WH6,7 	Hawaii
AH7K, KH7K,NH7K, WH7K 	Kure Island
AH8, KH8, NH8, WH8 	American Samoa
AH9, KH9, NH9, WH9 	Wake Island, Willis Island, Peake Island
KC4 	Antarctica
KG4 	Guantanamo Bay
KP1, NP1, WP1 	Navassa Island
KP2, KV4, NP2, WP2 	Virgin Island
KP4, NP4, WP4 	Puerto Rico (WP may also be used in a U.S. callsign)
